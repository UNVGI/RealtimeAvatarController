...
## dig ラウンド 4 確定事項の反映 (requirements.md 改訂内容)

### MotionFrame.timestamp 打刻責務 (Q10 = B)
受信ワーカースレッドは OSC パース完了後・`Subject.OnNext(frame)` 発行前に `Stopwatch.GetTimestamp() / (double)Stopwatch.Frequency` で `MotionFrame.timestamp` (double 秒) を打刻する。VMC 送信側 (バーチャルモーションキャプチャ等) が提供するタイムスタンプは使用しない (VMC v2.5 では不安定なため)。詳細は要件 2-7 を参照。

### VMCMoCapSourceConfig のランタイム動的生成 (Q11 = B)
`VMCMoCapSourceConfig` は `ScriptableObject.CreateInstance<VMCMoCapSourceConfig>()` によるランタイム動的生成を公式に許容する。受信ポート等のフィールドは `public` で直接セット可能。Factory は SO アセット経由 (シナリオ X) とランタイム動的生成 (シナリオ Y) のどちらでも同一コードパスで動作する (上書き不可)。詳細は要件 9-11・9-12 を参照。

### テスト asmdef 構成 (Q12 = C)
EditMode / PlayMode 2 系統の asmdef を用意する:
- `RealtimeAvatarController.MoCap.VMC.Tests.EditMode`: Config キャスト / 属性ベース自己登録確認 / OSC パーサ単体
- `RealtimeAvatarController.MoCap.VMC.Tests.PlayMode`: ローカル OSC ソケット経由の受信テスト (送信側テストダブル含む)

カバレッジ目標は初期版では設定しない。詳細は要件 10 を参照。
