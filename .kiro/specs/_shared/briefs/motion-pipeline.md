...
### dig ラウンド 4 確定事項
- **MotionFrame.Timestamp 仕様**: タイムスタンプは Stopwatch ベース monotonic (`double` 秒単位、App 起動基準)。`Stopwatch.GetTimestamp() / (double)Stopwatch.Frequency` で算出し、受信ワーカースレッド上で打刻する。プロセス間比較不可。将来ログ用途で `WallClock: DateTime?` フィールドの追加を検討するが、初期版では**未実装**
