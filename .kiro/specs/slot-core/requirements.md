...
8. **[dig ラウンド 4 追加]** The SlotSettings shall `ScriptableObject.CreateInstance<SlotSettings>()` によるランタイム動的生成をサポートする。生成後に各フィールドを直接セットするだけで有効なインスタンスとして `SlotManager` に渡せる (シナリオ Y)。SO アセット編集 (シナリオ X) とランタイム動的生成 (シナリオ Y) の両方で同一の `SlotManager` API を透過的に使用できる。
9. **[dig ラウンド 4 追加]** JSON 永続化 (ファイルへのエクスポート / インポート) は将来要件とし、本 Spec の初期実装では実装しない。ただし `SlotSettings` の構造は `JsonUtility` / Newtonsoft.Json による将来の JSON 永続化追加を妨げない設計とする。
...
8. **[dig ラウンド 4 追加]** The SlotManager shall `SlotSettings` インスタンスを引数に取って Slot を生成する API (`AddSlotAsync(SlotSettings settings)` 相当) を提供する。この API は SO アセット経由で取得した `SlotSettings` でも、`ScriptableObject.CreateInstance<SlotSettings>()` でランタイム動的生成した `SlotSettings` でも、区別なく受け付ける。API の正式シグネチャ (同期/非同期の選択、戻り値型等) は design フェーズで確定する。
9. **[dig ラウンド 4 追加]** The SlotManager の Slot 生成 API は、`SlotSettings` 内の各 Config (ProviderConfigBase 派生 / MoCapSourceConfigBase 派生 等) が SO アセット参照であるか `CreateInstance` で動的生成されたインスタンスであるかを問わず、同一の Factory / Registry 解決パスを通じて Slot を初期化できる。
...
### Requirement 14: テスト戦略 (EditMode / PlayMode 両系統) (dig ラウンド 4 追加)

**Objective:** As a Spec 実装者, I want slot-core の検証対象が EditMode / PlayMode の両系統でカバーされること, so that Unity ランタイム依存の有無に応じた適切なテスト境界を確立でき、CI での自動検証が可能になる。

#### 背景 (dig ラウンド 4 確定)

Unity の Test Framework は EditMode テストと PlayMode テストを別アセンブリ定義 (asmdef) で管理する。`slot-core` は Registry / Factory / Locator 等のドメインロジックを含むため、Unity エンジン起動を伴う PlayMode と伴わない EditMode の両方でテストを行う方針を採用する。カバレッジ目標は初期段階では設定せず、design / tasks フェーズで定量目標を設ける場合に改めて検討する。

#### Acceptance Criteria

1. The slot-core Spec shall EditMode テスト用アセンブリ定義 `RealtimeAvatarController.Core.Tests.EditMode` を提供する。EditMode テストは Unity エンジン起動を必要とせず、`IProviderRegistry` / `IMoCapSourceRegistry` の動作・Factory キャスト・`RegistryLocator.ResetForTest()` / `Override*()` による差し替え・`ISlotErrorChannel` のエラーストリーム配信を検証する。
2. The slot-core Spec shall PlayMode テスト用アセンブリ定義 `RealtimeAvatarController.Core.Tests.PlayMode` を提供する。PlayMode テストは Unity エンジン起動を伴い、実アバター上での Slot 動的追加・削除の最小限の統合検証を対象とする (具体的なテストケースは tasks フェーズで定義する)。
3. The EditMode テストの主な検証対象は以下とする:
   - `SlotRegistry` の追加・削除・重複登録エラー
   - `IProviderRegistry` / `IMoCapSourceRegistry` の Register / Resolve / GetRegisteredTypeIds
   - Factory 引数に基底クラス型インスタンスを渡した際のキャスト成功・失敗挙動
   - `RegistryLocator.ResetForTest()` によるインスタンスリセット
   - `RegistryLocator.Override*()` によるモック差し替え
   - `ISlotErrorChannel` に発行された `SlotError` の購読・内容確認
   - `SlotSettings` のランタイム動的生成 (`ScriptableObject.CreateInstance`) と各フィールドセット
4. The PlayMode テストの主な検証対象は以下とする (最小限):
   - `SlotManager` を介した Slot 動的追加・アクティブ化・削除の一連ライフサイクル
   - `MoCapSourceRegistry` の参照共有 (同一 Descriptor で複数 Slot に同一インスタンスが返ること)
5. The カバレッジ定量目標は初期段階では設定しない。design / tasks フェーズで必要に応じて再検討する。
6. The 各テスト asmdef は `RealtimeAvatarController.Core` アセンブリのみに依存し、具象実装 Spec (`mocap-vmc` 等) への依存を持たない。テスト用モック / スタブは各テスト asmdef 内に定義する。
