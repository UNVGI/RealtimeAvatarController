...
namespace RealtimeAvatarController.Core
{
    ...
    public static class RegistryLocator
    {
        ...
        /// <summary>IProviderRegistry への静的アクセスポイント。遅延初期化。</summary>
        public static IProviderRegistry ProviderRegistry
            => s_providerRegistry ?? (s_providerRegistry = new DefaultProviderRegistry());

        /// <summary>IMoCapSourceRegistry への静的アクセスポイント。遅延初期化。</summary>
        public static IMoCapSourceRegistry MoCapSourceRegistry
            => s_moCapSourceRegistry ?? (s_moCapSourceRegistry = new DefaultMoCapSourceRegistry());
        ...
        public static IFacialControllerRegistry FacialControllerRegistry
            => s_facialControllerRegistry ?? (s_facialControllerRegistry = new DefaultFacialControllerRegistry());

        /// <summary>ILipSyncSourceRegistry への静的アクセスポイント。遅延初期化 (将来用)。</summary>
        public static ILipSyncSourceRegistry LipSyncSourceRegistry
            => s_lipSyncSourceRegistry ?? (s_lipSyncSourceRegistry = new DefaultLipSyncSourceRegistry());

        /// <summary>ISlotErrorChannel への静的アクセスポイント。遅延初期化。</summary>
        public static ISlotErrorChannel ErrorChannel
            => s_errorChannel ?? (s_errorChannel = new DefaultSlotErrorChannel());
...
namespace RealtimeAvatarController.Core
{
    ...
    /// </summary>
    [Serializable]
    public sealed class AvatarProviderDescriptor
    {
        ...
        /// <summary>
        /// Resolve 時の等価性判定キー。typeId + Config の組み合わせ。
        /// MoCapSourceRegistry の参照共有判定に使用する。
        /// </summary>
        public bool Equals(AvatarProviderDescriptor other)
            => other != null && ProviderTypeId == other.ProviderTypeId && Config == other.Config;
    }

    /// <summary>MoCap ソースの Descriptor。</summary>
    [Serializable]
    public sealed class MoCapSourceDescriptor
    {
        ...
        public bool Equals(MoCapSourceDescriptor other)
            => other != null && SourceTypeId == other.SourceTypeId && Config == other.Config;
    }

    /// <summary>表情制御の Descriptor (null 許容)。</summary>
    [Serializable]
    public sealed class FacialControllerDescriptor
    {
        public string ControllerTypeId;
        public FacialControllerConfigBase Config;
    }

    /// <summary>リップシンクソースの Descriptor (null 許容)。</summary>
    [Serializable]
    public sealed class LipSyncSourceDescriptor
    {
        public string SourceTypeId;
...
namespace RealtimeAvatarController.Core
{
    ...
    {
        // UniRx の Subject を使用。スレッドセーフな Subject が必要な場合は Subject.Synchronize() を使用する。
...
**設計方針**:
- 静的フィールドにインスタンスを保持する。`lock` は使用せず、遅延初期化は null チェック + 代入のみ。
- マルチスレッド競合が発生しうるが、Registry は起動時 (`BeforeSceneLoad`) に登録が完了しており、その後の `Register()` 呼び出しは想定しないため許容する。
...
### 4.6 RegistryConflictException のスローポリシー
...
- `IMoCapSource` の受信エラー通知が受信スレッドから発行される場合は、`Publish()` 呼び出し前にメインスレッドへの移行を行う (`ObserveOnMainThread()` または UniTask の `SwitchToMainThread()` を使用)。
- `Subject<SlotError>` は `Subject.Synchronize()` でラップし、スレッドセーフ発行を保証する。
...
| `ApplyFailure` | モーション適用処理 (motion-pipeline / SlotManager) | `SlotManager` | FallbackBehavior 実行後に ErrorChannel に発行 |
| `VmcReceive` | `IMoCapSource` 具象実装 (mocap-vmc) | `IMoCapSource` 実装または SlotManager 経由 | MotionStream は OnError を発行しない。内部処理してストリーム継続 |
| `RegistryConflict` | `IProviderRegistry.Register()` / `IMoCapSourceRegistry.Register()` | Registry | `RegistryConflictException` をスロー。ErrorChannel にも発行 |
...
- `TPose`: T ポーズへ遷移。
- `Hide`: アバターの `GameObject.SetActive(false)` を呼ぶ。
- フォールバック後は `ISlotErrorChannel` に `ApplyFailure` を発行すること。
