...
1. The `mocap-movin` package shall MOVIN 専用の MotionFrame 派生型 (例: `MovinMotionFrame`) を本パッケージ内に定義し、本体パッケージが提供する MotionFrame 階層 (`RealtimeAvatarController.Core.MotionFrame` または `RealtimeAvatarController.Motion.MotionFrame`) のいずれかを継承元として採用する。`HumanoidMotionFrame` / `GenericMotionFrame` 等の Humanoid・Generic 具象は継承しない (具体的な継承元は design フェーズで確定する; `Motion.MotionFrame` を採用すると `Timestamp` / `SkeletonType` を再実装せずに済む利点がある)。
...
1. The `MovinMotionFrame` shall 要件 2.1 で確定された MotionFrame 階層上のいずれかの基底 (`RealtimeAvatarController.Core.MotionFrame` または `RealtimeAvatarController.Motion.MotionFrame`) を継承するイミュータブル型として定義する。
...
3. The `MovinMotionFrame` shall `/VMC/Ext/Root/Pos` 由来の追加情報 (localScale / localOffset) を任意 (nullable) として保持できる構造を持つ。これらは「Avatar GameObject 自身の Transform」ではなく、`/VMC/Ext/Root/Pos` の OSC 引数として送られてくる**ボーン名と紐付くデータ**として扱う (送信側で「root pose」とラベルされたボーン Transform に対して後段で適用される。Avatar GameObject の Transform は本フレームの書き込み対象ではない)。
4. The `MovinMotionFrame` shall タイムスタンプを Adapter ローカル (受信側) で打刻し、MOVIN 送信側の `/VMC/Ext/T` を直接タイムスタンプとして採用しない。打刻形式・プロパティ名は採用する基底型 (`Motion.MotionFrame` 継承時は基底プロパティ `Timestamp` の `Stopwatch.GetTimestamp() / (double)Stopwatch.Frequency` 仕様に従う; `Core.MotionFrame` 直接継承時は本パッケージ内で独自定義する) に従う。
...
7. The `MovinMotionApplier` shall `/VMC/Ext/Root/Pos` 由来の localScale が `MovinMotionFrame` に含まれる場合、当該 OSC 引数のボーン名で name 一致テーブルから解決された Transform に対してのみ `localScale` を書き込む。書き込み対象は **Avatar GameObject 自身の Transform ではなく**、name テーブルで解決されたボーン Transform である (このボーンは送信側で「root pose」とラベル付けされた骨格上の特定ボーンであり、Avatar 階層のルートと等しいとは限らない)。
...
1. The `MovinMoCapSource` shall `com.hidano.uosc.uOscServer` のスレッドモデル — UDP 受信は内部ワーカースレッドで行われるが `onDataReceived` の `Invoke` は MonoBehaviour `Update()` から `parser_.Dequeue()` 経由で**メインスレッド上で発火**する仕様 — を前提とする。本パッケージは uOSC 内部のワーカースレッドへ直接アクセスせず、`onDataReceived` ハンドラに購読するのみとする。
2. While `onDataReceived` ハンドラ内 (メインスレッド), the `MovinMoCapSource` shall `Subject.OnNext()` を即時呼び出さず、内部キャッシュ (ボーン名キーの最新値マップ) への書き込みのみ行う。これは 1 フレーム内に多数到来する OSC メッセージを 1 個の `MovinMotionFrame` に統合するため (フレームレート整合性のため) であり、スレッド境界クロスの解消が目的ではない。
3. The `MovinMoCapSource` shall Unity メインスレッドの Tick (例: `LateUpdate` / 共有 Tick ホスト) で内部キャッシュを snapshot 化して `MovinMotionFrame` を組み立て、`Subject.OnNext()` を発行する (具体的な Tick タイミングおよびホスト方式 — 単独 MonoBehaviour / 既存 `EVMC4USharedReceiver` 同等の共有 Receiver / その他 — は design フェーズで確定する)。
4. The `MovinMoCapSource.MotionStream` shall `OnError()` を一切発行しない (本体 `IMoCapSource` 契約に準拠)。
5. If MOVIN 受信処理中に Adapter 側で捕捉可能な例外が発生したとき, then the `MovinMoCapSource` shall `RegistryLocator.ErrorChannel.Publish()` を呼んで通知し、`MotionStream` をエラー終端させない。`SlotErrorCategory` は本体パッケージの既存定義を拡張せず流用する方針とし、OSC/VMC プロトコル系受信エラーは既存カテゴリ `VmcReceive` を「OSC 系受信エラー全般」と意味拡張して用いる (本体非改変制約の必然的帰結。具体カテゴリのマッピング表は design フェーズで確定する)。
6. If UDP ソケットのバインドに失敗したとき (ポート競合・権限不足等), then the `MovinMoCapSource.Initialize()` shall 例外をスローして呼び出し元に伝播する。本体側 `SlotManager` が捕捉して `ISlotErrorChannel` に通知する経路に乗せる。
7. The `MovinMoCapSource` shall `Debug.LogError` の抑制制御を自身で持たず、`DefaultSlotErrorChannel` が担う 1 フレーム抑制ロジックに委ねる。
8. The `MovinMoCapSource` shall マルチキャスト Observable として `MotionStream` を公開し、複数 Slot / 複数購読者から同一インスタンスを共有できる (`Publish().RefCount()` 等の具体実装は design フェーズで確定)。
