...
1. The `MovinMoCapSource` shall uOSC の `onDataReceived` がメインスレッドではなく**受信ワーカースレッド**上で発火することを前提とする (`com.hidano.uosc` は標準 uOSC 同様にワーカースレッドコールバックを採用しており、本パッケージはこの挙動に依存する)。
2. While 受信ワーカースレッドからのコールバック内, the `MovinMoCapSource` shall `Subject.OnNext()` を直接呼ばず、内部キャッシュ (ボーン名キーの最新値マップ) への書き込みのみ行う。キャッシュアクセスはスレッドセーフ性を保証する (具体的な同期方式 — `lock` / `ConcurrentDictionary` / 二重バッファ等 — は design フェーズで確定する)。
3. The `MovinMoCapSource` shall Unity メインスレッドの Tick (例: `Update` / `LateUpdate` / 共有 Tick ホスト) で内部キャッシュを snapshot 化して `MovinMotionFrame` を組み立て、`Subject.OnNext()` を**メインスレッド上で**発行する (具体的な Tick タイミングおよびホスト方式 — 単独 MonoBehaviour / 既存 `EVMC4USharedReceiver` 同等の共有 Receiver / その他 — は design フェーズで確定する)。
4. The `MovinMoCapSource.MotionStream` shall `OnError()` を一切発行しない (本体 `IMoCapSource` 契約に準拠)。
5. If MOVIN 受信処理中に Adapter 側で捕捉可能な例外が発生したとき, then the `MovinMoCapSource` shall `RegistryLocator.ErrorChannel.Publish()` を呼んで通知し、`MotionStream` をエラー終端させない (`SlotErrorCategory` の具体値は design フェーズで確定する)。
